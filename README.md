...
- `GET /api/fiat-to-crypto/orders/[id]`: owner order fetch + notification log (API remains; no progress UI page).
- `pages/orders/new.jsx` + `POST /api/fiat-to-crypto/orders` + `GET /api/fiat-to-crypto/quote-preview`: fiat-to-crypto **indicative quote** (no login) and **intake** after login (FCX-25, FCX-40).
- `pages/admin/orders.jsx` + `GET /api/fiat-to-crypto/admin/queue?status=submitted`: minimal **admin inbox** for new requests (fulfillment off-app).
- `src/domain/fiatToCryptoPricing.js` + related **admin APIs**: commission engine (FCX-22, FCX-42) — available for integrations; **no** pricing UI in this slim app.
- `src/domain/fiatToCryptoExecution.js` + related **admin APIs**: execution/retry and audit (FCX-24, FCX-44) — **no** execution UI in this slim app.
- `src/domain/fiatToCryptoDeposit.js` + deposit **API** routes (`POST …/deposits/reconcile`, `GET …/deposits/log`): engine and endpoints remain for scripts/integrations; **no deposit UI** in this app.
