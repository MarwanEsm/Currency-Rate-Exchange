...
- `src/domain/fiatToCryptoExecution.js` + `POST /api/fiat-to-crypto/admin/orders/[id]/execute-purchase`: liquidity-provider purchase with retry, audit log, and `purchasing → transferring | failed` order patch (FCX-24). The admin screen lists `purchasing` orders and triggers execution.
- `pages/admin/deposits.jsx` + `src/domain/fiatToCryptoDeposit.js` + `POST /api/fiat-to-crypto/admin/deposits/reconcile` + `GET /api/fiat-to-crypto/admin/deposits/log`: fiat deposit verification & reconciliation — matches deposits to submitted orders, verifies amount/currency with configurable tolerance, records `matched / amount_under / amount_over / currency_mismatch / no_matching_order / order_not_eligible / duplicate_deposit` events, and auto-transitions `submitted → paid` only on `matched` (FCX-23).
